...
void accumulateflow(Mat& flow, Mat& accflow)
{

    for (int i = 0; i < accflow.rows-1; i++) {
        for(int j = 0; j < accflow.cols-1; j++) {
            const Point2f& fxy = accflow.at< Point2f>(i,j);
            //std::cout << fxy <<flow.at< Point2f>(i,j)<<std::endl;
            //std::cout<<"Index is "<< i <<" "<< j << std::endl;
            //accflow.at< Point2f>(i, j) = accflow.at< Point2f>(i, j);
...
void calclongtimeDenseFlowGPU(string file_name, int bound, int type, int step, int dev_id,
    ...
    int cnt = 0;
    int framecount = 0;
    int fcorr = 10;//The number of frames to keep track of for one long time flow
    while(true){
